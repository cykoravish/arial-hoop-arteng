...
export function Services() {
  ...
  return (
    <section id="services" className="py-28 md:py-36 px-6 bg-surface" aria-labelledby="services-heading">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="text-center mb-20">
          <p className="font-body text-xs tracking-[0.3em] uppercase text-gold mb-6">{t.services.label}</p>
          <h2
            id="services-heading"
        ...
        <div className="grid md:grid-cols-3 gap-px bg-border">
          {t.services.items.map((service, i) => (
            ...
            >
              ...
              <h3 className="font-display text-3xl font-light text-foreground mb-4">{service.title}</h3>

              {/* Divider */}
              <div className={`w-10 h-px mb-6 transition-all duration-500 group-hover:w-16 ${i === 0 ? "bg-gold" : "bg-border"}`} />
              ...
              <ul className="space-y-2 mb-10" role="list">
                {service.features.map((feat) => (
                  <li key={feat} className="flex items-center gap-3 font-body text-sm text-muted-foreground">
